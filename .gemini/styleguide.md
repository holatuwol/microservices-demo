...
Variables should avoid numbered suffixes. Instead, use a prefix that describes the purpose of the variable.

```java
// String s1 = map.getString("roleId");
String roleIdString = map.getString("roleId");
```

---

Reduce indentation. Avoid nested if statements using early `return` or `continue` statements. Avoid loops with large method bodies by delegating to a function instead.
...
Initialize static variables immediately in the class declaration rather than inside of a method whenever possible to avoid accidental `null` values.

```java
public class MyClass {
  public static remember(String key, String value) {
    // if (myMap == null) {
  ...
  private static Map<String, String> myMap = new LinkedHashMap<>();
}
```
